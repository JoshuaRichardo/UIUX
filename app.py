...
def _stock_header(df_raw, ticker):
    ...
    active_period = st.session_state.chart_period

    # Inject per-button CSS — each button gets a unique class based on its key
    period_css = "<style>"
    for label in PERIODS:
        key = f"period_{label}_{ticker}"
        if label == active_period:
            period_css += f"""
            div[data-testid="stButton"] button[data-testid="baseButton-secondary"]#btn_{key},
            .pchip-{label.replace('+','p')} > div[data-testid="stButton"] > button {{
                background: #00D084 !important;
                color: #000000 !important;
                border: none !important;
                box-shadow: 0 3px 12px rgba(0,208,132,0.35) !important;
                font-weight: 700 !important;
            }}
            """
        else:
            ...
            .pchip-{label.replace('+','p')} > div[data-testid="stButton"] > button:hover {{
                background: #1B2A40 !important;
                color: #E2E8F0 !important;
            }}
            """
    # Common sizing for all period buttons
    period_css += """
    div[data-testid="stHorizontalBlock"] .pchip > div[data-testid="stButton"] > button,
    div[data-testid="stHorizontalBlock"] div[class*="pchip"] > div[data-testid="stButton"] > button {
    ...
    st.markdown(period_css, unsafe_allow_html=True)

    btn_cols = st.columns(len(PERIODS))
    for col, (label, days) in zip(btn_cols, PERIODS.items()):
        safe = label.replace('+', 'p')
        cls = f"pchip pchip-{safe}"
        with col:
            st.markdown(f'<div class="{cls}">', unsafe_allow_html=True)
            if st.button(label, key=f"period_{label}_{ticker}", use_container_width=True):
...
def _page_login_gate():
    """Full-screen login page shown before the main app."""
    st.markdown("""
    <style>
    /* Hide sidebar on login page */
    section[data-testid="stSidebar"] { display: none !important; }
    .block-container { max-width: 1100px !important; padding: 60px 3rem !important; }
    ...
    .login-gate-bg {
        ...
        background: #080E1A;
        background-image:
            radial-gradient(ellipse 70% 60% at 50% 0%, rgba(0,208,132,0.10) 0%, transparent 70%),
    ...
    .login-gate-headline span { color: #00D084; }
    .login-gate-sub {
        font-size: 15px;
        color: #64748B;
        line-height: 1.7;
    ...
    .login-gate-card {
        ...
        border-radius: 28px;
        padding: 36px 32px 28px;
        box-shadow: 0 20px 60px rgba(0,0,0,0.5), 0 0 0 1px rgba(0,208,132,0.05);
    }
    .login-gate-card-title {
        font-size: 22px;
        font-weight: 700;
        color: white;
        letter-spacing: -0.025em;
    ...
    @media (max-width: 768px) {
        .login-gate-split { flex-direction: column; gap: 36px; }
        .login-gate-brand { max-width: 100%; text-align: center; }
        .login-gate-logo { justify-content: center; }
        .login-gate-feat { justify-content: center; }
        .login-gate-card { flex: unset; width: 100%; max-width: 380px; }
        .login-gate-headline { font-size: 30px; }
    }
    </style>
    """, unsafe_allow_html=True)
    ...
    with left_col:
        ...
        <div style="padding: 40px 20px 40px 10px;">
          <div class="login-gate-logo" style="margin-bottom: 32px;">
            <div class="login-gate-logo-dot">📈</div>
            <div class="login-gate-logo-name">StockPredict AI</div>
          </div>
          <div class="login-gate-headline">
            Prediksi saham<br>dengan <span>kecerdasan AI</span>
          </div>
          <div class="login-gate-sub">
            Platform analisis saham berbasis machine learning untuk investor cerdas.
            Pantau portofolio, prediksi harga, dan raih keputusan investasi terbaik.
          </div>
          <div class="login-gate-feat"><div class="login-gate-feat-dot"></div><div class="login-gate-feat-text">Prediksi 5 hari ke depan dengan model LSTM</div></div>
          <div class="login-gate-feat"><div class="login-gate-feat-dot"></div><div class="login-gate-feat-text">Data real-time dari Yahoo Finance</div></div>
          <div class="login-gate-feat"><div class="login-gate-feat-dot"></div><div class="login-gate-feat-text">Analisis saham IDX &amp; global</div></div>
          <div class="login-gate-feat"><div class="login-gate-feat-dot"></div><div class="login-gate-feat-text">Signal BUY / SELL berbasis AI</div></div>
        </div>
        """, unsafe_allow_html=True)

    # ── Right side: login card — CSS wraps the Streamlit form ───────────────
    with right_col:
        # Inject CSS to style the st.form as a card
        st.markdown("""
        <style>
        [data-testid="stForm"] {
            ...
            box-shadow: 0 20px 60px rgba(0,0,0,0.5), 0 0 0 1px rgba(0,208,132,0.05) !important;
        ...
        with st.form("gate_login_form", clear_on_submit=False):
            ...
            <div style="margin-bottom:20px;">
              <div style="font-size:22px; font-weight:700; color:white; letter-spacing:-0.025em; margin-bottom:6px; font-family:'DM Sans',sans-serif;">Masuk ke akun kamu</div>
              <div style="font-size:13.5px; color:#64748B; font-family:'DM Sans',sans-serif;">Gunakan email atau username untuk melanjutkan</div>
              <div style="height:1px; background:rgba(255,255,255,0.05); margin-top:20px;"></div>
            </div>
            """, unsafe_allow_html=True)

            st.markdown('<p style="font-size:12px; color:#94A3B8; margin-bottom:4px; font-family:\'DM Sans\',sans-serif; font-weight:500;">Email atau Username</p>', unsafe_allow_html=True)
            ...
            st.markdown('<p style="font-size:12px; color:#94A3B8; margin: 12px 0 4px; font-family:\'DM Sans\',sans-serif; font-weight:500;">Password</p>', unsafe_allow_html=True)
            ...
            col_chk, col_lupa = st.columns([1, 1])
            with col_chk:
                st.checkbox("Ingat saya")
            with col_lupa:
                st.markdown('<div style="text-align:right; padding-top:6px;"><a href="#" style="color:#00D084; font-size:12px; text-decoration:none; font-family:\'DM Sans\',sans-serif;">Lupa password?</a></div>', unsafe_allow_html=True)
            ...
            if submitted:
                if username and password:
                    ...
                    st.error("⚠️ Email/username dan password wajib diisi.")

        st.markdown("""
        <div style="text-align:center; color:#64748B; font-size:13px; margin-top:16px; font-family:'DM Sans',sans-serif;">
          Belum punya akun?
          <a href="#" style="color:#00D084; font-weight:600; text-decoration:none;">Daftar gratis</a>
        </div>
        """, unsafe_allow_html=True)


def main():
    # ── GATE: Tampilkan login dulu sebelum semua konten ───────────────────────
